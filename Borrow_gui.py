...
label=t.Label(window,text="Welcome to Library!")
label.grid(row=0,sticky="w" and "s",padx=20,pady=30)
...
def validate():
    ...
    if firstName.isalpha() and lastName.isalpha():
        ...
        filename="F:\python projects\LMS\Borrow-"+firstName+" "+lastName+".txt"
        ...
        try:   
            ...
            try:
                ...
                if(int(ListSplit.quantity[a])>0):
                    ...
                    scrollt.insert(t.INSERT,str(book_counter)+"."+ ListSplit.bookname[a]+"-"+ListSplit.authorname[a]+"\n")

                    #writing borrowed book details to borrowers file
                    with open(filename,"a") as f:
                        f.write(str(book_counter)+". \t\t"+ ListSplit.bookname[a]+"\t\t  "+ListSplit.authorname[a]+"\n")
